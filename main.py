...
async def on_message(message):
    ...
    if (os.getenv("DEBUG_MODE") == "1") and not (message.channel.id == DEBUG_CHANNNEL_ID or message.channel.id == SRAN_RESULT_CHANNNEL_ID):
        return
    ...
    if message.attachments and (message.channel.id == SRAN_RESULT_CHANNNEL_ID):
        images = []
        ...
        result_analyze_module = get_result_analyze_module()
        response_text = result_analyze_module.analyze_result_with_gemini(images)
