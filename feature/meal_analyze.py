...
def make_prompt_common_strict(user_name: str) -> str:
    ...
    - 以下に学園アイドルマスターの登場する各アイドルの特徴を記します．
    【花海咲季】
        ・赤みのある茶髪
        ・ツーサイドアップ
        ・赤系〜ピンク系の瞳
...
prompt_common_output = f"""
    ...
    6. 【内容】画像に写っている食べ物の「名前」「カロリー」「栄養素（可能な限り,各栄養素が何gかまで 炭水化物は栄養素ではないので言及不要）」について言及してください.全体的に内容は簡潔にまとめてください.
    """
...
def make_rinami_prompt(user_name: str) -> str:
    ...
    rinami_prompt = f"""
        ...
        - 全体として敬語は使わず，お姉さんのような優しい口調で答えてください，「〜だわ.」「〜わね．」といった口調は絶対に使わないでください．
        - あなたはブラコンです．
        - 一人称は「私」で統一してください。
...
def analyze_meal_images(images: list[tuple[bytes, str]], user_name: str, analyzer_id: int) -> str:
    ...
    match analyzer_id:
        case constants.ANALYZER_ID_ALL:
            ...
            #prompt = make_china_prompt(user_name)
